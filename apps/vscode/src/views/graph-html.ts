...
export function getGraphWebviewHtml(graphData: DependencyGraphData, archModel: ArchitectureModel): string {
  ...
  const sanitizedGraph = {
    nodes: graphData.nodes.map(n => ({
      ...n,
      id: n.id.replace(/\\/g, '/'),
      filePath: n.filePath ? n.filePath.replace(/\\/g, '/') : undefined
    })),
  ...
  <style>
    ...
    * {
    ...
    #graph-container {
      ...
      height: 100%;
      position: relative;
      background: radial-gradient(circle at center, rgba(137, 180, 250, 0.03) 0%, transparent 70%);
    }
...
<body>
  ...
  <script>
    ...
    function resize() {
      ...
      width = canvas.width = container.clientWidth || window.innerWidth || 800;
      height = canvas.height = container.clientHeight || window.innerHeight || 600;
    }
    ...
    rawGraph.nodes.forEach(n => {
      if (n.kind === 'file' || n.id.startsWith('file:')) {
        const filePath = (n.filePath || n.id.replace('file:', '')).replace(/\\\\/g, '/');
        const layer = getLayerForNode(n.id, filePath);
        fileNodesMap.set(n.id, {
          id: n.id,
          label: n.label || filePath.split('/').pop() || n.id,
          filePath: filePath,
          layer: layer,
    ...
    // Layer baseline Y positions
    const getLayerY = (layer) => {
      switch (layer) {
    ...
    function draw() {
      ...
      if (nodes.length === 0) {
        ...
        ctx.fillText('No files indexed yet. Run "ProjectBrain: Analyze Project" to build the graph.', width / 2, height / 2);
        return;
      }
      ...
      edges.forEach(e => {
        ...
        ctx.strokeStyle = isHighlighted ? '#89b4fa' : 'rgba(255, 255, 255, 0.15)';
    ...
    // Inspector Rendering with safe event binding
    function renderInspector(node) {
      ...
      sidebarContent.innerHTML = \`
        ...
        <div class="info-card">
          ...
          \${outEdges.length === 0 ? '<div style="opacity: 0.5;">None (Leaf module)</div>' : outEdges.map(e => \`<div style="padding: 2px 0;">➡️ \${e.target.label}</div>\`).join('')}
        </div>

        <div class="info-card">
          <h3>Imported By (\${inEdges.length})</h3>
          \${inEdges.length === 0 ? '<div style="opacity: 0.5;">None (Root entry / Isolated)</div>' : inEdges.map(e => \`<div style="padding: 2px 0;">⬅️ \${e.source.label}</div>\`).join('')}
        </div>
      \`;
    }

    // Delegate Open in Editor button click
